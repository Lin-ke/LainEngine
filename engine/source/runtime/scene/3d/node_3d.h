...
private:
	// For the sake of ease of use, GObject3D can operate with Transforms (Basis+Origin), Quaternion/Scale and Euler Rotation/Scale.
	...
	//
	// All this is transparent from outside the GObject3D API, which allows everything to works by calling these functions in exchange.
	//
	...
	mutable SelfList<GObject> xform_change;
	...
	struct Data {
		...
		GObject3D *parent = nullptr;
		List<GObject3D *> children;
		List<GObject3D *>::Element *C = nullptr;
...
#ifdef TOOLS_ENABLED
		Vector<Ref<GObject3DGizmo>> gizmos;
	...
	GObjectPath visibility_parent_path;
	...
	void _update_gizmos();
	void _notify_dirty();
	void _propagate_transform_changed(GObject3D *p_origin);
...
public:
	...
	Transform3D get_relative_transform(const GObject *p_parent) const;
	...
	void set_visibility_parent(const GObjectPath &p_path);
