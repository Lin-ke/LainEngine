...
static int _mouse_mode_to_glfw(Input::MouseMode p_mode) {
  switch (p_mode) {
    case Input::MOUSE_MODE_VISIBLE:
      return GLFW_CURSOR_NORMAL;
    case Input::MOUSE_MODE_HIDDEN:
      return GLFW_CURSOR_HIDDEN;
    case Input::MOUSE_MODE_CAPTURED:
      return GLFW_CURSOR_HIDDEN;
    case Input::MOUSE_MODE_CONFINED:
      return GLFW_CURSOR_HIDDEN;
    case Input::MOUSE_MODE_CONFINED_HIDDEN:
      return GLFW_CURSOR_HIDDEN;
  }
...
Point2i WindowSystem::_get_screens_origin() const {
  ...
  EnumPosData data = {0, 0, Point2()};  // 会把data 和信息都传到_MonitorEnumProcOrigin里
...
WindowSystem::WindowID WindowSystem::NewWindow(const WindowCreateInfo* create_info) {
  ...
  glfwSetInputMode(window, GLFW_CURSOR, _mouse_mode_to_glfw(m_mouse_mode));
  ...
  registerOnCursorPosFunc(std::bind(&WindowSystem::on_cursor_pos, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), id);
  registerOnCursorEnterFunc(std::bind(&WindowSystem::on_cursor_enter, this, std::placeholders::_1, std::placeholders::_2), id);
  registerOnScrollFunc(std::bind(&WindowSystem::on_scroll, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), id);
  registerOnMouseButtonFunc(std::bind(&WindowSystem::on_mouse_button, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), id);
  registerOnWindowCloseFunc(std::bind(&WindowSystem::on_window_close, this, std::placeholders::_1), id);
  registerOnKeyFunc(std::bind(&WindowSystem::on_key, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5),
                    id);
...
void WindowSystem::_dispatch_input_event(const Ref<InputEvent>& p_event) {
	...
	// dispatch 
	for(const KeyValue<WindowID, WindowData>& E : m_windows){
		if(p_event.is_valid() && E.value.m_input_event_func){
			E.value.m_input_event_func(p_event);
...
void WindowSystem::_dispatch_input_events(const Ref<InputEvent>& p_event) {
	 { static_cast<WindowSystem*>(GetSingleton())->_dispatch_input_event(p_event); }
}

WindowSystem::WindowSystem(const String& p_rendering_driver, WindowMode p_mode, VSyncMode p_vsync_mode, uint32_t p_flags, const Vector2i* p_position,
                           const Vector2i& p_resolution, int p_screen, Error& r_error) {
  p_singleton = this;
  ...
  if (rendering_context) {
	...
	Input::set_mouse_mode_func = _set_mouse;
	Input::get_mouse_mode_func = _get_mouse;
}
...
void WindowSystem::mouse_set_mode(Input::MouseMode p_mode) {
  _THREAD_SAFE_METHOD_
...
void WindowSystem::_set_mouse(Input::MouseMode p_mode) {
	p_singleton->mouse_set_mode(p_mode);
}

Input::MouseMode WindowSystem::_get_mouse() {
	return p_singleton->mouse_get_mode();
}
...
Size2i WindowSystem::window_get_position(WindowSystem::WindowID p_window) const
{
	_THREAD_SAFE_METHOD_

	ERR_FAIL_COND_V(!m_windows.has(p_window), Point2i());
	const WindowData &wd = m_windows[p_window];
...
bool WindowSystem::window_is_focused(WindowSystem::WindowID p_window) const {
	_THREAD_SAFE_METHOD_
	ERR_FAIL_COND_V(!m_windows.has(p_window), false);
	const WindowData &wd = m_windows[p_window];
...
void WindowSystem::on_cursor_pos(int id, double xpos, double ypos) {
  ...
  if (window_mouseover_id != over_id) {  // mouse enter
    window_mouseover_id = over_id;
    // Once-off notification, must call again.
    // track_mouse_leave_event(hWnd);
  }
  if (m_mouse_mode != Input::MOUSE_MODE_CAPTURED) {
    return;
  }
  ...
  mm->set_global_position(Point2(xpos, ypos));
	if (m_mouse_mode == Input::MOUSE_MODE_CAPTURED) {
				Point2i c(m_windows[id].width / 2, m_windows[id].height / 2);
				old_x = c.x;
				old_y = c.y;
				...
				ClientToScreen(m_windows[id].hWnd, &pos);
...
void WindowSystem::cursor_set_shape(Input::CursorShape p_shape) {
  ...
  if (m_mouse_mode != Input::MOUSE_MODE_VISIBLE && m_mouse_mode != Input::MOUSE_MODE_CONFINED) {
    cursor_shape = p_shape;
    return;
  }

  static const LPCTSTR win_cursors[Input::CURSOR_MAX] = {IDC_ARROW,  IDC_IBEAM,
                                                         IDC_HAND,  // Finger.
                                                         IDC_CROSS,  IDC_WAIT,     IDC_APPSTARTING, IDC_SIZEALL, IDC_ARROW,  IDC_NO,     IDC_SIZENS,
                                                         IDC_SIZEWE, IDC_SIZENESW, IDC_SIZENWSE,    IDC_SIZEALL, IDC_SIZENS, IDC_SIZEWE, IDC_HELP};
...
void WindowSystem::on_mouse_button(int id, int button, int action, int mod) {  // 按下鼠标
                                                                               // 左上角是(0,0)
