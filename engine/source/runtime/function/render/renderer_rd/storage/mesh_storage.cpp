...
#include "function/render/rendering_system/renderer_compositor_storage.h"
...
void MeshStorage::mesh_add_surface(RID p_mesh, const RS::SurfaceData &p_surface) {
	...
	{
		uint32_t stride = 0; // 
		uint32_t attrib_stride = 0; // color, uv, 
	...
	if (new_surface.index_count) {
		// index buffer的 字节数 根据vertex count 来看。
		bool is_index_16 = new_surface.vertex_count <= 65536 && new_surface.vertex_count > 0; // 0x FFFF (2^16-1 = 65535)
...
void MeshStorage::_mesh_instance_add_surface(MeshInstance *mi, Mesh *mesh, uint32_t p_surface) {
	...
	// mesh->surfaces[p_surface] 就是这个surface
...
// uniform 添加 vertex buffer， blend weights buffer
// see skeleton.glsl
...
RS::SurfaceData MeshStorage::mesh_get_surfacedata(RID p_mesh, int p_surface) const {
	Mesh *mesh = mesh_owner.get_or_null(p_mesh);
...
void MeshStorage::multimesh_set_mesh(RID p_multimesh, RID p_mesh) {
	...
	if (multimesh->data_cache.size()) {
		//we have a data cache, just mark it dirty @typo
