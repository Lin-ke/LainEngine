...
void SceneShaderForwardClustered::ShaderData::set_code(const String& p_code) {
  ...
  actions.entry_point_stages["vertex"] = shader::ShaderCompiler::STAGE_VERTEX;
  actions.entry_point_stages["fragment"] = shader::ShaderCompiler::STAGE_FRAGMENT;
  actions.entry_point_stages["light"] = shader::ShaderCompiler::STAGE_FRAGMENT;
...
#endif
  shader_singleton->shader.version_set_code(version, gen_code.code, gen_code.uniforms, gen_code.stage_globals[shader::ShaderCompiler::STAGE_VERTEX],
  ...
  for (int i = 0; i < CULL_VARIANT_MAX; i++) {
    RD::PolygonCullMode cull_mode_rd_table[CULL_VARIANT_MAX][3] = {{RD::POLYGON_CULL_DISABLED, RD::POLYGON_CULL_FRONT, RD::POLYGON_CULL_BACK},
    ...
    for (int j = 0; j < RS::PRIMITIVE_MAX; j++) {
      RD::RenderPrimitive primitive_rd_table[RS::PRIMITIVE_MAX] = {
          RD::RENDER_PRIMITIVE_POINTS, RD::RENDER_PRIMITIVE_LINES, RD::RENDER_PRIMITIVE_LINESTRIPS, RD::RENDER_PRIMITIVE_TRIANGLES, RD::RENDER_PRIMITIVE_TRIANGLE_STRIPS,
      };
      ...
      for (int k = 0; k < PIPELINE_VERSION_MAX; k++) {
        ...
        if (k == PIPELINE_VERSION_COLOR_PASS) {
          for (int l = 0; l < PIPELINE_COLOR_PASS_FLAG_COUNT; l++) {
            ...
            color_pipelines[i][j][l].setup(shader_variant, primitive_rd, raster_state, multisample_state, depth_stencil, blend_state, 0,
                                           singleton->default_specialization_constants);
          }
        } else {
          RD::PipelineColorBlendState blend_state;
          RD::PipelineDepthStencilState depth_stencil = depth_stencil_state;
...
SceneShaderForwardClustered::ShaderData::ShaderData() : shader_list_element(this) {}
...
bool SceneShaderForwardClustered::MaterialData::update_parameters(const HashMap<StringName, Variant>& p_parameters, bool p_uniform_dirty, bool p_textures_dirty) {
  ...
  return update_parameters_uniform_set(p_parameters, p_uniform_dirty, p_textures_dirty, shader_data->uniforms, shader_data->ubo_offsets.ptr(), shader_data->texture_uniforms,
                                       ...
                                       shader_singleton->shader.version_get_shader(shader_data->version, 0), RenderForwardClustered::MATERIAL_UNIFORM_SET, true, true);
}
...
void SceneShaderForwardClustered::init(const String p_defines) {
  ...
  {  // 在这里进行了变体和组的定义
    ...
    shader_versions.push_back(
        ShaderRD::VariantDefine(SHADER_GROUP_BASE, "\n#define MODE_RENDER_DEPTH\n#define MODE_DUAL_PARABOLOID\n", true));  // SHADER_VERSION_DEPTH_PASS_DP
    ...
    shader_versions.push_back(ShaderRD::VariantDefine(
    ...
    for (int i = 0; i < SHADER_COLOR_PASS_FLAG_COUNT; i++) {  // color pass flag 所有可能的组合
      String version = "";                                    // 这里传入的第一个是什么都没有的
