...
void ShaderRD::setup(const String& p_vertex_code, const String& p_fragment_code, const String& p_compute_code, const String& p_name) {
  ...
  tohash.append("1");
  tohash.append("[SpirvCacheKey]");  // spirv
...
void ShaderRD::_compile_variant(uint32_t p_variant, const CompileData* p_data) {
  ...
  if (!build_ok) {
    ...
    ERR_PRINT("Error compiling " + String(current_stage == RD::SHADER_STAGE_COMPUTE ? "Compute " : (current_stage == RD::SHADER_STAGE_VERTEX ? "Vertex" : "Fragment")) +
              " shader, variant #" + itos(variant) + " (" + variant_defines[variant].text.get_data() + ").");
    ERR_PRINT(error);
...
// custum defines 是在用户shader文件里的 defines
...
#endif
        builder.append(String("#define RENDER_DRIVER_") + OS::GetSingleton()->GetCurrentRenderingDriverName().to_upper() + "\n");
      } break;
...
void ShaderRD::_compile_version(Version* p_version, int p_group) {
  ...
  WorkerThreadPool::GroupID group_task = WorkerThreadPool::get_singleton()->add_template_group_task(
      this, &ShaderRD::_compile_variant, &compile_data, group_to_variant_map[p_group].size(), -1, true, SNAME("ShaderCompilation"));
...
String ShaderRD::_get_cache_file_path(Version* p_version, int p_group) {
  ...
  const String& path = shader_cache_dir.path_join(name).path_join(group_sha256[p_group]).path_join(sha1) + "." + api_safe_name + ".cache";  // vulkan.cache
  return path;
}
// cache与shader binary不同
static const char* shader_file_header = "LNSC";
...
void ShaderRD::version_set_code(RID p_version, const HashMap<String, String>& p_code, const String& p_uniforms, const String& p_vertex_globals,
                                const String& p_fragment_globals, const Vector<String>& p_custom_defines) {
