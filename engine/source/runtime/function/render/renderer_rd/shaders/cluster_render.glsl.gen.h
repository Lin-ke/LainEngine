...
public:
	...
	ClusterRenderShaderRD() {
		...
		Error err;
		Ref<FileAccess> file1 = FileAccess::open("D:/LainEngine/engine/source/runtime/function/render/renderer_rd/shaders/cluster_render.glsl.gen.vertex.lglsl", FileAccess::READ,  &err);
		DEV_ASSERT(err == OK);
		static const String _vertex_code = file1->get_as_utf8_string();
		Ref<FileAccess> file2 = FileAccess::open("D:/LainEngine/engine/source/runtime/function/render/renderer_rd/shaders/cluster_render.glsl.gen.fragment.lglsl", FileAccess::READ,  &err);
		DEV_ASSERT(err == OK);
