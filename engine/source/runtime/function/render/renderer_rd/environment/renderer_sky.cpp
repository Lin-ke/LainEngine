...
// Our info in our UBO is only used if we're rendering stereo.
	...
	sky_scene_state.ubo.fog_density = 0;
	sky_scene_state.ubo.fog_aerial_perspective = 0;
	Color fog_color = {0,0,0,1};
	float fog_energy = 0;
	sky_scene_state.ubo.fog_light_color[0] = fog_color.r * fog_energy;
	sky_scene_state.ubo.fog_light_color[1] = fog_color.g * fog_energy;
	sky_scene_state.ubo.fog_light_color[2] = fog_color.b * fog_energy;
	sky_scene_state.ubo.fog_sun_scatter = 0;

	sky_scene_state.ubo.fog_sky_affect = 0;
	sky_scene_state.ubo.volumetric_fog_sky_affect = 0;
	// copy
...
void lain::RendererRD::SkyRD::update_res_buffers(Ref<RenderSceneBuffersRD> p_render_buffers, RID p_env, double p_time, float p_luminance_multiplier) {
	ERR_FAIL_COND(p_render_buffers.is_null());
	RendererRD::MaterialStorage *material_storage = RendererRD::MaterialStorage::get_singleton();
	ERR_FAIL_COND(p_env.is_null());
...
void lain::RendererRD::SkyRD::_render_sky(RD::DrawListID p_list, float p_time, RID p_fb, PipelineCacheRD* p_pipeline, RID p_uniform_set, RID p_texture_set,
                                          const Projection& p_projection, const Basis& p_orientation, const Vector3& p_position, float p_luminance_multiplier) {
