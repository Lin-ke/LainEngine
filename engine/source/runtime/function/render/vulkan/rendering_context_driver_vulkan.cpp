...
#define USE_ANY_VALIDATION_LAYER 1
...
VKAPI_ATTR VkBool32 VKAPI_CALL RenderingContextDriverVulkan::_debug_messenger_callback(VkDebugUtilsMessageSeverityFlagBitsEXT p_message_severity, VkDebugUtilsMessageTypeFlagsEXT p_message_type, const VkDebugUtilsMessengerCallbackDataEXT* p_callback_data, void* p_user_data) {
	...
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
		...
		CRASH_COND_MSG(Engine::GetSingleton()->is_abort_on_gpu_errors_enabled(), "Crashing, because abort on GPU errors is enabled.");
		break;
...
void RenderingContextDriverVulkan::surface_set_vsync_mode(SurfaceID p_surface, WindowSystem::VSyncMode p_vsync_mode) {
	Surface* surface = (Surface*)(p_surface);
	surface->vsync_mode = p_vsync_mode;
	surface->needs_resize = true;
}

lain::WindowSystem::VSyncMode RenderingContextDriverVulkan::surface_get_vsync_mode(SurfaceID p_surface) const {
	Surface* surface = (Surface*)(p_surface);
...
/// </summary>
/// <param name="p_physical_device"></param>
/// <param name="p_queue_family_index"></param>
...
Error RenderingContextDriverVulkan::initialize() {
	...
	err = _initialize_instance_extensions(); // 必须的拓展
	ERR_FAIL_COND_V(err != OK, err);

	err = _initialize_instance(); // create instance, debug，函数绑定
	ERR_FAIL_COND_V(err != OK, err);

	err = _initialize_devices(); // 获得物理设备及物理设备支持的queue family
	ERR_FAIL_COND_V(err != OK, err);
...
Error RenderingContextDriverVulkan::_initialize_vulkan_version() {
	...
	if (func != nullptr) {
		...
		L_DERROR("vkEnumerateInstanceVersion not available, assuming Vulkan 1.3.");
...
/// </summary>
/// <param name="r_layer_names"></param>
/// <returns></returns>
	...
	if (instance_layer_count > 0) {
		...
		for (const std::initializer_list<const char*>& list : { preferred, lunarg, google }) {
			...
			if (layers_found) { // all find (otherwise it will break)
				r_layer_names.reserve(list.size()); // 加入 r_layer_names
				for (const char* layer_name : list) {
...
/// <summary>
/// create application, create instance, vkGetInstanceProcAddr bind functions
/// </summary>
/// <returns></returns>
Error RenderingContextDriverVulkan::_initialize_instance() {
	Error err;

	CharString cs = GLOBAL_GET("application/config/name").operator lain::String().utf8();
	VkApplicationInfo app_info{};
	app_info.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
	app_info.pApplicationName = cs;
	...
	TightLocalVector<const char*> enabled_extension_names; // 复制一下
	...
	if (has_debug_utils_extension) {
		...
		debug_messenger_create_info.pUserData = this; // 这个userdata塞一个指针的写法还是很常见的
...
bool RenderingContextDriverVulkan::_use_validation_layers() const {
	return Engine::GetSingleton()->is_validation_layers_enabled();
}
