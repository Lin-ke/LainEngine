...
// 布局转换时 textureid用谁的是无所谓的，切片(vkcreateimageview) 中都保存了 原始的vkimage
...
//           }
//         }
//         resource_tracker->usage = parent_traker->usage; // 第一次见到这个slice，用父亲现在的属性给他
//         resource_tracker->usage_access = parent_traker->usage_access;
...
//       int32_t read_slice_cmd_idx = search_tracker->read_slice_cmd_idx;
//       while (read_slice_cmd_idx >= 0) {
//         const RecordedSliceListNode& read_list_node =
//             read_slice_list_nodes[read_slice_cmd_idx];
...
//             DEV_ASSERT(search_tracker->read_slice_cmd_idx == read_slice_cmd_idx);
//             search_tracker->read_slice_cmd_idx = read_list_node.next_list_index;
//           }

//           // Advance to the next element.
//           read_slice_cmd_idx = read_list_node.next_list_index;
//         } else {
//           previous_slice_command_list_index = read_slice_cmd_idx;
//           read_slice_cmd_idx = read_list_node.next_list_index;
//         }
...
void RenderingDeviceGraph::_add_command_to_graph(ResourceTracker** p_resource_trackers, ResourceUsage* p_resource_usages, uint32_t p_resource_count, int32_t p_command_index,
  ...
  for (uint32_t i = 0; i < p_resource_count; i++) {
    ...
    if (is_resource_a_slice) {
      ...
      if (resource_tracker->parent->usage == RESOURCE_USAGE_NONE) {
        if (resource_tracker->parent->texture_driver_id.id != 0) {
          // If the resource is a texture, we transition it entirely to the layout determined by the first slice that uses it.
          _add_texture_barrier_to_command(resource_tracker->parent->texture_driver_id, RDD::BarrierAccessBits(0), new_usage_access, RDG::RESOURCE_USAGE_NONE,
                                          new_resource_usage, resource_tracker->parent->texture_subresources, command_normalization_barriers,
                                          r_command->normalization_barrier_index, r_command->normalization_barrier_count);
        }
        ...
        if (resource_tracker->parent->usage == new_resource_usage) {
          ...
          while (current_tracker != nullptr) {
            ...
            if (current_tracker == resource_tracker) {
              ...
              if (initialized_dirty_rect) {
                resource_tracker->parent->slice_or_dirty_rect = resource_tracker->parent->slice_or_dirty_rect.merge(current_tracker->slice_or_dirty_rect);
              } else {
                resource_tracker->parent->slice_or_dirty_rect = current_tracker->slice_or_dirty_rect;
                initialized_dirty_rect = true;
              }
        ...
        if (resource_tracker->parent->dirty_shared_list != nullptr && resource_tracker->parent->slice_or_dirty_rect.intersects(resource_tracker->slice_or_dirty_rect)) {
          // There's an intersection with the current dirty area of the parent and the slice. We must verify if the intersection is against a slice
          // that was used in this command or not. Any slice we can find that wasn't used by this command must be reverted to the layout of the parent.
          ResourceTracker* previous_tracker = nullptr;
          ResourceTracker* current_tracker = resource_tracker->parent->dirty_shared_list;
          ...
          while (current_tracker != nullptr) {
            ...
            if (current_tracker->slice_or_dirty_rect.intersects(resource_tracker->slice_or_dirty_rect)) {
              if (current_tracker->command_frame == tracking_frame && current_tracker->slice_cmd_idx == p_command_index) {
                ERR_FAIL_MSG("Texture slices that overlap can't be used in the same command.");
              } else {
                // Delete the slice from the dirty list and revert it to the usage of the parent.
                if (current_tracker->texture_driver_id.id != 0) {
                  _add_texture_barrier_to_command(current_tracker->texture_driver_id, current_tracker->usage_access, new_usage_access, current_tracker->usage,
                                                  resource_tracker->parent->usage, current_tracker->texture_subresources, command_normalization_barriers,
                                                  r_command->normalization_barrier_index, r_command->normalization_barrier_count);

                  // Merge the area of the slice with the current tracking area of the command and indicate it's a write usage as well.
                  search_tracker_rect = search_tracker_rect.merge(current_tracker->slice_or_dirty_rect);
                  write_usage = true;
                }
        ...
        if (resource_tracker->usage != new_resource_usage) {
          ...
          if (resource_tracker->parent->dirty_shared_list != nullptr) {
            resource_tracker->parent->slice_or_dirty_rect = resource_tracker->parent->slice_or_dirty_rect.merge(resource_tracker->slice_or_dirty_rect);
          } else {
            resource_tracker->parent->slice_or_dirty_rect = resource_tracker->slice_or_dirty_rect;
          }
        }
      }
    } else {
      ...
      while (current_tracker != nullptr) {
        ...
        if (current_tracker->texture_driver_id.id != 0) {
          // Transition all slices to the layout of the parent resource.
          _add_texture_barrier_to_command(current_tracker->texture_driver_id, current_tracker->usage_access, new_usage_access, current_tracker->usage, resource_tracker->usage,
                                          current_tracker->texture_subresources, command_normalization_barriers, r_command->normalization_barrier_index,
                                          r_command->normalization_barrier_count);
        }
    ...
    if (different_usage || write_usage_after_write) {
      ...
      if (resource_tracker->texture_driver_id.id != 0) {
        ...
        _add_texture_barrier_to_command(resource_tracker->texture_driver_id, resource_tracker->usage_access, new_usage_access, resource_tracker->usage, new_resource_usage,
                                        resource_tracker->texture_subresources, command_transition_barriers, r_command->transition_barrier_index,
                                        r_command->transition_barrier_count);
        RenderingDeviceDriverVulkan::TextureInfo info = *(RenderingDeviceDriverVulkan::TextureInfo*)resource_tracker->texture_driver_id.id;
        // L_PRINT("add_texture_barrier_to_image " + itos((int)info.vk_view_create_info.image) + "from usage" + itos((int)resource_tracker->usage) + "to usage" + itos((int)new_resource_usage));
      } else if (resource_tracker->buffer_driver_id.id != 0) {
        // #if USE_BUFFER_BARRIERS
        // _add_buffer_barrier_to_command(resource_tracker->buffer_driver_id, resource_tracker->usage_access, new_usage_access, RDD::BufferRange(), r_command->buffer_barrier_index, r_command->buffer_barrier_count);
        // #endif
        // FIXME: Memory barriers are currently pushed regardless of whether buffer barriers are being used or not. Refer to the comment on the
        // definition of USE_BUFFER_BARRIERS for the reason behind this. This can be fixed to be one case or the other once it's been confirmed
        // the buffer and memory barrier behavior discrepancy has been solved.
    ...
    if (search_tracker->write_cmd_idx >= 0) {
      if (search_tracker->write_cmd_list_enable) {
        // Make this command adjacent to any commands that wrote to this resource and intersect with the slice if it applies.
        // For buffers or textures that never use slices, this list will only be one element long at most.
        int32_t previous_write_list_index = -1;
        int32_t write_list_index = search_tracker->write_cmd_idx;
    ...
    if (write_usage) {
      if (resource_has_parent) {
        if (!search_tracker->write_cmd_list_enable && search_tracker->write_cmd_idx >= 0) {
          ...
          search_tracker->write_cmd_idx = _add_to_write_list(search_tracker->write_cmd_idx, tracker_rect, -1);
        }
      ...
      int32_t read_slice_cmd_idx = search_tracker->read_slice_cmd_idx;
      while (read_slice_cmd_idx >= 0) {
        const RecordedSliceListNode& read_list_node = read_slice_list_nodes[read_slice_cmd_idx];
        if (!resource_has_parent || search_tracker_rect.encloses(read_list_node.subresources)) {
          if (previous_slice_command_list_index >= 0) {
            // Erase this element and connect the previous one to the next element.
      ...
      // We add a read dependency to the tracker to indicate this command reads from the resource slice.
      search_tracker->read_slice_cmd_idx = _add_to_slice_read_list(p_command_index, resource_tracker_rect, search_tracker->read_slice_cmd_idx);
    } else {
      // We add a read dependency to the tracker to indicate this command reads from the entire resource.
      search_tracker->read_full_cmd_idx = _add_to_command_list(p_command_index, search_tracker->read_full_cmd_idx);
    }
  }
}
...
#if USE_BUFFER_BARRIERS
void RenderingDeviceGraph::_add_buffer_barrier_to_command(RDD::BufferID p_buffer_id, BitField<RDD::BarrierAccessBits> p_src_access,
                                                          BitField<RDD::BarrierAccessBits> p_dst_access, RDD::BufferRange p_range, int32_t& r_barrier_index,
...
void RenderingDeviceGraph::_add_texture_barrier_to_command(RDD::TextureID p_texture_id, BitField<RDD::BarrierAccessBits> p_src_access,
                                                           ...
                                                           RDD::TextureSubresourceRange p_subresources, LocalVector<RDD::TextureBarrier>& r_barrier_vector,
...
void RenderingDeviceGraph::initialize(RDD* p_driver, RenderingContextDriver::Device p_device, uint32_t p_frame_count,
                                      RDD::CommandQueueFamilyID p_secondary_command_queue_family, uint32_t p_secondary_command_buffers_per_frame) {
  driver = p_driver;
  device = p_device;
...
void RenderingDeviceGraph::add_buffer_copy(RDD::BufferID p_src, ResourceTracker* p_src_tracker, RDD::BufferID p_dst, ResourceTracker* p_dst_tracker,
...
void RenderingDeviceGraph::add_compute_list_bind_pipeline(RDD::PipelineID p_pipeline) {
  ComputeListBindPipelineInstruction* instruction =
      reinterpret_cast<ComputeListBindPipelineInstruction*>(_allocate_compute_list_instruction(sizeof(ComputeListBindPipelineInstruction)));
...
void RenderingDeviceGraph::add_compute_list_bind_uniform_set(RDD::ShaderID p_shader, RDD::UniformSetID p_uniform_set, uint32_t set_index) {
  ComputeListBindUniformSetInstruction* instruction =
      reinterpret_cast<ComputeListBindUniformSetInstruction*>(_allocate_compute_list_instruction(sizeof(ComputeListBindUniformSetInstruction)));
...
void RenderingDeviceGraph::add_compute_list_dispatch_indirect(RDD::BufferID p_buffer, uint32_t p_offset) {
  ComputeListDispatchIndirectInstruction* instruction =
      reinterpret_cast<ComputeListDispatchIndirectInstruction*>(_allocate_compute_list_instruction(sizeof(ComputeListDispatchIndirectInstruction)));
...
void RenderingDeviceGraph::add_compute_list_uniform_set_prepare_for_use(RDD::ShaderID p_shader, RDD::UniformSetID p_uniform_set, uint32_t set_index) {
  ComputeListUniformSetPrepareForUseInstruction* instruction =
      reinterpret_cast<ComputeListUniformSetPrepareForUseInstruction*>(_allocate_compute_list_instruction(sizeof(ComputeListUniformSetPrepareForUseInstruction)));
...
void RenderingDeviceGraph::add_compute_list_end() {
  ...
  _add_command_to_graph(compute_instruction_list.command_trackers.ptr(), compute_instruction_list.command_tracker_usages.ptr(),
...
void RenderingDeviceGraph::add_draw_list_begin(RDD::RenderPassID p_render_pass, RDD::FramebufferID p_framebuffer, Rect2i p_region,
...
void RenderingDeviceGraph::add_draw_list_bind_index_buffer(RDD::BufferID p_buffer, RDD::IndexBufferFormat p_format, uint32_t p_offset) {
  DrawListBindIndexBufferInstruction* instruction =
      reinterpret_cast<DrawListBindIndexBufferInstruction*>(_allocate_draw_list_instruction(sizeof(DrawListBindIndexBufferInstruction)));
...
void RenderingDeviceGraph::add_draw_list_bind_uniform_set(RDD::ShaderID p_shader, RDD::UniformSetID p_uniform_set, uint32_t set_index) {
  DrawListBindUniformSetInstruction* instruction =
      reinterpret_cast<DrawListBindUniformSetInstruction*>(_allocate_draw_list_instruction(sizeof(DrawListBindUniformSetInstruction)));
...
void RenderingDeviceGraph::add_draw_list_bind_vertex_buffers(VectorView<RDD::BufferID> p_vertex_buffers, VectorView<uint64_t> p_vertex_buffer_offsets) {
  ...
  uint32_t instruction_size =
      sizeof(DrawListBindVertexBuffersInstruction) + sizeof(RDD::BufferID) * p_vertex_buffers.size() + sizeof(uint64_t) * p_vertex_buffer_offsets.size();
...
void RenderingDeviceGraph::add_draw_list_clear_attachments(VectorView<RDD::AttachmentClear> p_attachments_clear, VectorView<Rect2i> p_attachments_clear_rect) {
  uint32_t instruction_size =
      sizeof(DrawListClearAttachmentsInstruction) + sizeof(RDD::AttachmentClear) * p_attachments_clear.size() + sizeof(Rect2i) * p_attachments_clear_rect.size();
...
void RenderingDeviceGraph::add_draw_list_execute_commands(RDD::CommandBufferID p_command_buffer) {
  DrawListExecuteCommandsInstruction* instruction =
      reinterpret_cast<DrawListExecuteCommandsInstruction*>(_allocate_draw_list_instruction(sizeof(DrawListExecuteCommandsInstruction)));
...
void RenderingDeviceGraph::add_draw_list_set_blend_constants(const Color& p_color) {
  DrawListSetBlendConstantsInstruction* instruction =
      reinterpret_cast<DrawListSetBlendConstantsInstruction*>(_allocate_draw_list_instruction(sizeof(DrawListSetBlendConstantsInstruction)));
...
void RenderingDeviceGraph::add_draw_list_uniform_set_prepare_for_use(RDD::ShaderID p_shader, RDD::UniformSetID p_uniform_set, uint32_t set_index) {
  DrawListUniformSetPrepareForUseInstruction* instruction =
      reinterpret_cast<DrawListUniformSetPrepareForUseInstruction*>(_allocate_draw_list_instruction(sizeof(DrawListUniformSetPrepareForUseInstruction)));
...
void RenderingDeviceGraph::add_draw_list_end() {
  ...
  _add_command_to_graph(draw_instruction_list.command_trackers.ptr(), draw_instruction_list.command_tracker_usages.ptr(), draw_instruction_list.command_trackers.size(),
...
void RenderingDeviceGraph::add_texture_copy(RDD::TextureID p_src, ResourceTracker* p_src_tracker, RDD::TextureID p_dst, ResourceTracker* p_dst_tracker,
...
void RenderingDeviceGraph::add_texture_get_data(RDD::TextureID p_src, ResourceTracker* p_src_tracker, RDD::BufferID p_dst,
                                                VectorView<RDD::BufferTextureCopyRegion> p_buffer_texture_copy_regions, ResourceTracker* p_dst_tracker) {
  DEV_ASSERT(p_src_tracker != nullptr);
...
void RenderingDeviceGraph::add_texture_resolve(RDD::TextureID p_src, ResourceTracker* p_src_tracker, RDD::TextureID p_dst, ResourceTracker* p_dst_tracker,
                                               uint32_t p_src_layer, uint32_t p_src_mipmap, uint32_t p_dst_layer, uint32_t p_dst_mipmap) {
  DEV_ASSERT(p_src_tracker != nullptr);
  DEV_ASSERT(p_dst_tracker != nullptr);
...
void RenderingDeviceGraph::begin_label(const String& p_label_name, const Color& p_color) {
  ...
  command_label_count++;
  command_label_string = p_label_name;
}

void RenderingDeviceGraph::end_label() {
  command_label_index = -1;
  command_label_string = "default_name";
}
  ...
  while (instruction_data_cursor < p_instruction_data_size) {
    ...
    switch (instruction->type) {
      ...
      case ComputeListInstruction::TYPE_BIND_UNIFORM_SET: {
        ...
        driver->command_bind_compute_uniform_set(p_command_buffer, bind_uniform_set_instruction->uniform_set, bind_uniform_set_instruction->shader,
                                                 bind_uniform_set_instruction->set_index);
      ...
      case ComputeListInstruction::TYPE_SET_PUSH_CONSTANT: {
        ...
        const VectorView push_constant_data_view(reinterpret_cast<const uint32_t*>(set_push_constant_instruction->data()),
      ...
      case ComputeListInstruction::TYPE_UNIFORM_SET_PREPARE_FOR_USE: {
        const ComputeListUniformSetPrepareForUseInstruction* uniform_set_prepare_for_use_instruction =
  ...
  while (instruction_data_cursor < p_instruction_data_size) {
    ...
    switch (instruction->type) {
      case DrawListInstruction::TYPE_BIND_INDEX_BUFFER: {
        ...
        driver->command_render_bind_index_buffer(p_command_buffer, bind_index_buffer_instruction->buffer, bind_index_buffer_instruction->format,
                                                 bind_index_buffer_instruction->offset);
      ...
      case DrawListInstruction::TYPE_BIND_UNIFORM_SET: {
        ...
        driver->command_bind_render_uniform_set(p_command_buffer, bind_uniform_set_instruction->uniform_set, bind_uniform_set_instruction->shader,
                                                bind_uniform_set_instruction->set_index);
      ...
      case DrawListInstruction::TYPE_BIND_VERTEX_BUFFERS: {
        ...
        driver->command_render_bind_vertex_buffers(p_command_buffer, bind_vertex_buffers_instruction->vertex_buffers_count, bind_vertex_buffers_instruction->vertex_buffers(),
      ...
      case DrawListInstruction::TYPE_DRAW_INDEXED: {
        ...
        driver->command_render_draw_indexed(p_command_buffer, draw_indexed_instruction->index_count, draw_indexed_instruction->instance_count,
...
void RenderingDeviceGraph::_run_render_command(int32_t p_level, const RecordedCommandSort* p_sorted_commands, uint32_t p_sorted_commands_count,
                                               RDD::CommandBufferID& r_command_buffer, CommandBufferPool& r_command_buffer_pool, int32_t& r_current_label_index,
  ...
  for (uint32_t i = 0; i < p_sorted_commands_count; i++) {
    ...
    _run_label_command_change(r_command_buffer, command->label_index, p_level, false, true, &p_sorted_commands[i], p_sorted_commands_count - i, r_current_label_index,
    ...
    switch (command->type) {
      ...
      case RecordedCommand::TYPE_DRAW_LIST: {
        ...
        driver->command_begin_render_pass(r_command_buffer, draw_list_command->render_pass, draw_list_command->framebuffer, draw_list_command->command_buffer_type,
                                          draw_list_command->region, clear_values);
      ...
      case RecordedCommand::TYPE_TEXTURE_CLEAR: {
        ...
        driver->command_clear_color_texture(r_command_buffer, texture_clear_command->texture, RDD::TEXTURE_LAYOUT_COPY_DST_OPTIMAL, texture_clear_command->color,
                                            texture_clear_command->range);
      } break;
      case RecordedCommand::TYPE_TEXTURE_COPY: {
        ...
        const VectorView<RDD::TextureCopyRegion> command_texture_copy_regions_view(texture_copy_command->texture_copy_regions(),
                                                                                   texture_copy_command->texture_copy_regions_count);
        driver->command_copy_texture(r_command_buffer, texture_copy_command->from_texture, RDD::TEXTURE_LAYOUT_COPY_SRC_OPTIMAL, texture_copy_command->to_texture,
      ...
      case RecordedCommand::TYPE_TEXTURE_GET_DATA: {
        ...
        const VectorView<RDD::BufferTextureCopyRegion> command_buffer_texture_copy_regions_view(texture_get_data_command->buffer_texture_copy_regions(),
                                                                                                texture_get_data_command->buffer_texture_copy_regions_count);
        driver->command_copy_texture_to_buffer(r_command_buffer, texture_get_data_command->from_texture, RDD::TEXTURE_LAYOUT_COPY_SRC_OPTIMAL,
      ...
      case RecordedCommand::TYPE_TEXTURE_RESOLVE: {
        ...
        driver->command_resolve_texture(r_command_buffer, texture_resolve_command->from_texture, RDD::TEXTURE_LAYOUT_RESOLVE_SRC_OPTIMAL, texture_resolve_command->src_layer,
      ...
      case RecordedCommand::TYPE_TEXTURE_UPDATE: {
        ...
        for (uint32_t j = 0; j < texture_update_command->buffer_to_texture_copies_count; j++) {
          driver->command_copy_buffer_to_texture(r_command_buffer, command_buffer_to_texture_copies[j].from_buffer, texture_update_command->to_texture,
                                                 RDD::TEXTURE_LAYOUT_COPY_DST_OPTIMAL, command_buffer_to_texture_copies[j].region);
        }
      } break;
...
void RenderingDeviceGraph::_run_label_command_change(RDD::CommandBufferID p_command_buffer, int32_t p_new_label_index, int32_t p_new_level, bool p_ignore_previous_value,
                                                     bool p_use_label_for_empty, const RecordedCommandSort* p_sorted_commands, uint32_t p_sorted_commands_count,
...
void RenderingDeviceGraph::end(bool p_reorder_commands, bool p_full_barriers, RDD::CommandBufferID& r_command_buffer, CommandBufferPool& r_command_buffer_pool) {
  ...
  if (p_reorder_commands) {  //
    ...
    for (int32_t i = 0; i < command_count; i++) {
      ...
      commands_sorted[sorted_command_index].priority = RecordedCommandSort::PriorityTable[recorded_command->type];
    }
  ...
  if (p_reorder_commands) {
    ...
    for (uint32_t i = 0; i < command_count; i++) {
      if (current_level != commands_sorted[i].level) {  // new level
        ...
        _run_render_command(current_level, level_command_ptr, level_command_count, r_command_buffer, r_command_buffer_pool, current_label_index, current_label_level);
    ...
    for (uint32_t i = 0; i < command_count; i++) {
      ...
      _run_render_command(i, &commands_sorted[i], 1, r_command_buffer, r_command_buffer_pool, current_label_index, current_label_level);
    }
  }
...
void RenderingDeviceGraph::_group_barriers_for_render_commands(RDD::CommandBufferID p_command_buffer, const RecordedCommandSort* p_sorted_commands,
  ...
  const VectorView<RDD::TextureBarrier> texture_barriers =
      barrier_group.normalization_barriers.is_empty() ? barrier_group.transition_barriers : barrier_group.normalization_barriers;
#if USE_BUFFER_BARRIERS
...
void RenderingDeviceGraph::_print_render_commands(const RecordedCommandSort* p_sorted_commands, uint32_t p_sorted_commands_count) {
  for (uint32_t i = 0; i < p_sorted_commands_count; i++) {
    ...
    switch (command->type) {
      ...
      case RecordedCommand::TYPE_BUFFER_COPY: {
        const RecordedBufferCopyCommand* buffer_copy_command = reinterpret_cast<const RecordedBufferCopyCommand*>(command);
        print_line(command_index, "LEVEL", command_level, "BUFFER COPY SOURCE", itos(buffer_copy_command->source.id), "DESTINATION",
      ...
      case RecordedCommand::TYPE_BUFFER_UPDATE: {
        const RecordedBufferUpdateCommand* buffer_update_command = reinterpret_cast<const RecordedBufferUpdateCommand*>(command);
        print_line(command_index, "LEVEL", command_level, "BUFFER UPDATE DESTINATION", itos(buffer_update_command->destination.id), "COPIES",
      ...
      case RecordedCommand::TYPE_TEXTURE_RESOLVE: {
        const RecordedTextureResolveCommand* texture_resolve_command = reinterpret_cast<const RecordedTextureResolveCommand*>(command);
        print_line(command_index, "LEVEL", command_level, "TEXTURE RESOLVE FROM", itos(texture_resolve_command->from_texture.id), "TO",
      ...
      case RecordedCommand::TYPE_CAPTURE_TIMESTAMP: {
        const RecordedCaptureTimestampCommand* texture_capture_timestamp_command = reinterpret_cast<const RecordedCaptureTimestampCommand*>(command);
        print_line(command_index, "LEVEL", command_level, "CAPTURE TIMESTAMP POOL", itos(texture_capture_timestamp_command->pool.id), "INDEX",
