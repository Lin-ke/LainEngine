...
// span; .._view; 或者slice
// 为了结合Vector与LocalVector，在const&中使用：
...
class VectorView {
 ...
 public:
  ...
  VectorView(const Vector<T>& p_lv) : _ptr(p_lv.ptr()), _size(p_lv.size()) {}
...
class RenderingDeviceDriver : public RenderingDeviceCommons {
 ...
 public:
  // 这里的ID都是指向graphicapi_driver内部的指针
  // 这些RID的生命周期管理需要手动进行 （这）
  // Handle
  struct HashMapHasherID {
    template <typename T>
    static uint32_t hash(const T& p_id) {
      return HashMapHasherDefault::hash(p_id.id);
    }
  };
  struct ID {
    size_t id = 0;
    _ALWAYS_INLINE_ ID() = default;
    _ALWAYS_INLINE_ ID(size_t p_id) : id(p_id) {}
  };
...
#define DEFINE_ID(m_name)                                                     \
 ...
 public:
  ...
  struct TextureView {
    DataFormat format = DATA_FORMAT_MAX;           // Format
    TextureSwizzle swizzle_r = TEXTURE_SWIZZLE_R;  // component mapping https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkComponentMapping.html
  ...
  enum TextureAspect {
    ...
    TEXTURE_ASPECT_METADATA = 3,
    IMAGE_ASPECT_PLANE_0 = 4,  // vulkan 1.1
  ...
  enum TextureAspectBits {
    ...
    TEXTURE_ASPECT_METADATA_BIT = (1 << TEXTURE_ASPECT_METADATA),
    IMAGE_ASPECT_PLANE_0_BIT = (1 << IMAGE_ASPECT_PLANE_0),  // vulkan 1.1
    IMAGE_ASPECT_PLANE_1_BIT = (1 << IMAGE_ASPECT_PLANE_1),
    IMAGE_ASPECT_PLANE_2_BIT = (1 << IMAGE_ASPECT_PLANE_2),
  };
  ...
  enum BarrierAccessBits {
    ...
    BARRIER_ACCESS_COLOR_ATTACHMENT_WRITE_BIT = (1 << 8), // 256
    BARRIER_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT = (1 << 9), // 512
    BARRIER_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT = (1 << 10), // 1024
  ...
  enum QueueIndices { QUEUE_INDEX_GRAPHICS, QUEUE_INDEX_COMPUTE, QUEUE_INDEX_TRANSFER, QUEUE_SPARSE_BINDING_BIT, QUEUE_PROTECTED_BIT, QUEUE_MAX };

  enum CommandQueueFamilyBits {
    COMMAND_QUEUE_FAMILY_GRAPHICS_BIT = 0x1,  // QUEUE_GRAPHICS_BIT used for vkCmdDraw* and graphic pipeline commands.
    COMMAND_QUEUE_FAMILY_COMPUTE_BIT = 0x2,   // used for vkCmdDispatch* and vkCmdTraceRays* and compute pipeline related commands.
    COMMAND_QUEUE_FAMILY_TRANSFER_BIT = 0x4,  // used for all transfer commands.
    COMMAND_QUEUE_SPARSE_BINDING_BIT = 0x8,   // used for binding sparse resources to memory with vkQueueBindSparse.
    COMMAND_QUEUE_PROTECTED_BIT = 0x10,       // used for protected memory.
  };
 ...
 public:
  ...
  virtual Error command_queue_execute_and_present(CommandQueueID p_cmd_queue, VectorView<SemaphoreID> p_wait_semaphores, VectorView<CommandBufferID> p_cmd_buffers,
                                                  VectorView<SemaphoreID> p_cmd_semaphores, FenceID p_cmd_fence, VectorView<SwapChainID> p_swap_chains) = 0;
 ...
 public:
  ...
  virtual void command_copy_texture(CommandBufferID p_cmd_buffer, TextureID p_src_texture, TextureLayout p_src_texture_layout, TextureID p_dst_texture,
                                    TextureLayout p_dst_texture_layout, VectorView<TextureCopyRegion> p_regions) = 0;
  virtual void command_resolve_texture(CommandBufferID p_cmd_buffer, TextureID p_src_texture, TextureLayout p_src_texture_layout, uint32_t p_src_layer, uint32_t p_src_mipmap,
                                       TextureID p_dst_texture, TextureLayout p_dst_texture_layout, uint32_t p_dst_layer, uint32_t p_dst_mipmap) = 0;
  virtual void command_clear_color_texture(CommandBufferID p_cmd_buffer, TextureID p_texture, TextureLayout p_texture_layout, const Color& p_color,
                                           const TextureSubresourceRange& p_subresources) = 0;

  virtual void command_copy_buffer_to_texture(CommandBufferID p_cmd_buffer, BufferID p_src_buffer, TextureID p_dst_texture, TextureLayout p_dst_texture_layout,
                                              VectorView<BufferTextureCopyRegion> p_regions) = 0;
  virtual void command_copy_texture_to_buffer(CommandBufferID p_cmd_buffer, TextureID p_src_texture, TextureLayout p_src_texture_layout, BufferID p_dst_buffer,
                                              VectorView<BufferTextureCopyRegion> p_regions) = 0;
  /******************/
  /**** PIPELINE ****/
  /******************/
  ...
  // subpass主要规定了attachement的格式

  enum AttachmentLoadOp {
    ATTACHMENT_LOAD_OP_LOAD = 0,   // 向前内容将被保留为初始值；read
    ATTACHMENT_LOAD_OP_CLEAR = 1,  // write
    ATTACHMENT_LOAD_OP_DONT_CARE = 2,
  };

  enum AttachmentStoreOp {
    ATTACHMENT_STORE_OP_STORE = 0,          // 指定渲染通道期间生成的内容以及渲染区域内的内容写入内存
    ATTACHMENT_STORE_OP_DONT_CARE = 1,      // 指定渲染区域内的内容渲染后不再需要，可以丢弃；附件的内容在渲染区域内将是未定义的。
    ATTACHMENT_STORE_OP_NONE = 1000301000,  // vk1.3,指定只要在渲染过程中没有将值写入附件，
                                            // 存储操作就不会访问渲染区域内的内容。
    // 如果在渲染过程中写入值，则其行为与 VK_ATTACHMENT_STORE_OP_DONT_CARE 相同，并且具有匹配的访问语义。
  };
  // 也许可以屏蔽掉attachment这一步
  // 直接把 Renderpass的ImageView都指定好
  ...
  virtual RenderPassID render_pass_create(VectorView<Attachment> p_attachments, VectorView<Subpass> p_subpasses, VectorView<SubpassDependency> p_subpass_dependencies,
  ...
  virtual void command_render_draw_indexed(CommandBufferID p_cmd_buffer, uint32_t p_index_count, uint32_t p_instance_count, uint32_t p_first_index, int32_t p_vertex_offset,
  ...
  virtual void command_render_draw_indexed_indirect_count(CommandBufferID p_cmd_buffer, BufferID p_indirect_buffer, uint64_t p_offset, BufferID p_count_buffer,
                                                          uint64_t p_count_buffer_offset, uint32_t p_max_draw_count, uint32_t p_stride) = 0;
  ...
  // multi_indirect
  virtual void command_render_draw_indirect_count(CommandBufferID p_cmd_buffer, BufferID p_indirect_buffer, uint64_t p_offset, BufferID p_count_buffer,
                                                  uint64_t p_count_buffer_offset, uint32_t p_max_draw_count, uint32_t p_stride) = 0;
  ...
  virtual PipelineID render_pipeline_create(ShaderID p_shader, VertexFormatID p_vertex_format, RenderPrimitive p_render_primitive,
                                            ...
                                            PipelineDepthStencilState p_depth_stencil_state, PipelineColorBlendState p_blend_state, VectorView<int32_t> p_color_attachments,
                                            BitField<PipelineDynamicStateFlags> p_dynamic_state, RenderPassID p_render_pass, uint32_t p_render_subpass,
  ...
  /**** QUERIES ****/
  /*****************/
  // https://zhuanlan.zhihu.com/p/349808392
  // ----- TIMESTAMP -----
