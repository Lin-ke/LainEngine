...
#include "function/render/scene/renderer_scene_occlusion_cull.h"
...
lain::RendererViewport::RendererViewport() {
	// @bug 提高健壮性需要clamp
	occlusion_rays_per_thread = GLOBAL_GET("rendering/occlusion_culling/occlusion_rays_per_thread");
	occlusion_rays_per_thread = CLAMP(occlusion_rays_per_thread,0,1024);
}
...
void RendererViewport::_configure_3d_render_buffers(Viewport* p_viewport) {
  if (p_viewport->render_buffers.is_valid()) {
    if (p_viewport->size.width() == 0 || p_viewport->size.height() == 0) {
      ...
      if ((!upscaler_available || scaling_3d_mode == RS::VIEWPORT_SCALING_3D_MODE_BILINEAR || scaling_3d_mode == RS::VIEWPORT_SCALING_3D_MODE_FSR) &&
          scaling_3d_scale >= (1.0 - EPSILON) && scaling_3d_scale <= (1.0 + EPSILON)) {
      ...
      int target_width;
      int target_height;
      int render_width;  // 渲染尺寸是 target_size * scaling_3d_scale, max 16384 (16k)
      int render_height;
...
void RendererViewport::viewport_attach_to_screen(RID p_viewport, const Rect2& p_rect, WindowSystem::WindowID p_screen) {
  ASSERT_VP_NOT_NULL;
  if (p_screen != WindowSystem::INVALID_WINDOW_ID) {
...
void RendererViewport::draw_viewports(bool p_swap_buffers) {
  ...
  HashMap<WindowSystem::WindowID, Vector<BlitToScreen>> blit_to_screen_list;
  //draw viewports
  ...
  int vertices_drawn = 0;
    ...
    if (vp->viewport_to_screen != WindowSystem::INVALID_WINDOW_ID 
    && (!vp->viewport_render_direct_to_screen || !RSG::rasterizer->is_low_end())) {
...
int lain::RendererViewport::get_total_primitives_drawn() const {
  return total_vertices_drawn;
}

int lain::RendererViewport::get_total_draw_calls_used() const {
...
void lain::RendererViewport::_draw_3d(Viewport* p_viewport) {
	...
	RSG::scene->render_camera(p_viewport->render_buffers, p_viewport->camera, p_viewport->scenario, p_viewport->self, p_viewport->internal_size, p_viewport->jitter_phase_count, screen_mesh_lod_threshold, p_viewport->shadow_atlas, &p_viewport->render_info);
...
// 这里似乎是为了low_end服务的
//https://docs.godotengine.org/zh-cn/4.x/classes/class_renderingserver.html#class-renderingserver-method-viewport-set-render-direct-to-screen
