...
class RenderingDevice : public RenderingDeviceCommons {
 ...
 public:
  ...
  // Some APIs (e.g., Vulkan) specifies a really complex behavior for the
  // application in order to tell when descriptor sets need to be re-bound (or
  // not). "When binding a descriptor set (see Descriptor Set Binding) to set
  ...
  //  then performing this binding does not disturb any of the lower numbered
  //  sets. If, additionally, the previous bound descriptor set for set N was
  ...
  // As a result, we need to figure out quickly when something is no longer
 ...
 private:
  // static const uint32_t MAX_UNIFORM_SETS = 32; 在父类中有声明
 ...
 private:
...
#endif
    // Actual pipeline.
    // 这个pipeline并不包括pipeline的一些细节
    RID shader;
    RDD::ShaderID shader_driver_id;
    uint32_t shader_layout_hash = 0;
    Vector<uint32_t> set_formats;  // 这个有什么用
    RDD::PipelineID driver_id;
    BitField<RDD::PipelineStageBits> stage_bits;
    uint32_t push_constant_size = 0;  // 这个size有什么用
  };
  ...
  struct ComputePipeline {
    ...
    uint32_t local_group_size[3] = {0, 0, 0};  // 区别是local group
  };  // 可能有更多
 ...
 public:
  ...
  struct Frame {
    ...
    // Used for filling up newly created buffers with data provided on
    // creation.
    ...
    // Signaled by the draw submission. Must wait on this fence before
    // beginning
    ...
    RDD::QueryPoolID timestamp_pool;  // // 应该是用来查询性能表现的
 ...
 public:
  ...
  enum MemoryType { MEMORY_TEXTURES, MEMORY_BUFFERS, MEMORY_TOTAL };
