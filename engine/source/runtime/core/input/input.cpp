...
void Input::VelocityTrack::reset() {
	last_tick = OS::GetSingleton()->GetTicksUsec();
	velocity = Vector2();
	accum = Vector2();
	accum_t = 0;
}
...
bool Input::is_action_just_pressed(const StringName &p_action, bool p_exact) const {
  ...
  return pressed_requirement && E->value.pressed_process_frame == Engine::GetSingleton()->get_process_frames();
}
...
bool Input::is_action_just_released(const StringName &p_action, bool p_exact) const {
  ...
  return released_requirement && E->value.released_process_frame == Engine::GetSingleton()->get_process_frames();
}
...
void Input::_parse_input_event_impl(const Ref<InputEvent> &p_event, bool p_is_emulated) {
	...
	for (const KeyValue<StringName, InputMap::Action> &E : InputMap::get_singleton()->get_action_map()) {
		...
		if (action_state.cache.pressed && !was_pressed) {
			action_state.pressed_physics_frame = Engine::GetSingleton()->get_physics_frames() + 1;
			action_state.pressed_process_frame = Engine::GetSingleton()->get_process_frames();
		}
		if (!action_state.cache.pressed && was_pressed) {
			action_state.released_physics_frame = Engine::GetSingleton()->get_physics_frames() + 1;
			action_state.released_process_frame = Engine::GetSingleton()->get_process_frames();
		}
	}
...
void Input::start_joy_vibration(int p_device, float p_weak_magnitude, float p_strong_magnitude, float p_duration) {
	...
	vibration.timestamp = OS::GetSingleton()->GetTicksUsec();
...
void Input::vibrate_handheld(int p_duration_ms, float p_amplitude) {
	// OS::GetSingleton()->vibrate_handheld(p_duration_ms, p_amplitude);
}
...
BitField<MouseButtonMask> Input::get_mouse_button_mask() const {
	return mouse_button_mask; // do not trust OS implementation, should remove it - OS::GetSingleton()->get_mouse_button_state();
}
...
#ifdef DEBUG_ENABLED
	uint64_t curr_frame = Engine::GetSingleton()->get_process_frames();
...
Vector<int> Input::get_connected_joypads() {
	Vector<int> ret;
...
Input::Input() {
	...
	String env_mapping = OS::GetSingleton()->GetEnvironment("SDL_GAMECONTROLLERCONFIG");
	...
	String env_ignore_devices = OS::GetSingleton()->GetEnvironment("SDL_GAMECONTROLLER_IGNORE_DEVICES");
