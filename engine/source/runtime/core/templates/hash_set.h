...
template <class TKey, class Hasher = HashMapHasherDefault, class Comparator = HashMapComparatorDefault<TKey>>
class HashSet {
 public:
 ...
 public:
  ...
  struct Iterator {
    _FORCE_INLINE_ const TKey& operator*() const { return keys[index]; }
    _FORCE_INLINE_ const TKey* operator->() const { return &keys[index]; }
    ...
    _FORCE_INLINE_ explicit operator bool() const { return keys != nullptr; }
  ...
  _FORCE_INLINE_ Iterator begin() const { return num_elements ? Iterator(keys, num_elements, 0) : Iterator(); }
  _FORCE_INLINE_ Iterator end() const { return Iterator(); }
  ...
  HashSet(const HashSet& p_other) { _init_from(p_other); }
  ...
  HashSet() { capacity_index = MIN_CAPACITY_INDEX; }
