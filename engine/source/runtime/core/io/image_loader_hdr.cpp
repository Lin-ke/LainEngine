...
Error ImageLoaderHDR::load_image(Ref<Image> p_image, Ref<FileAccess> f, BitField<ImageLoader::LoaderFlags> p_flags, float p_scale) {
...

	{
		...
		for (int i = 0; i < width * height; i++) {
			...
			if (p_flags & ImageLoader::FLAG_FORCE_LINEAR) {
				c = c.srgb_to_linear();
			}
...
void lain::ImageLoaderHDR::get_recognized_extensions(List<String>* p_extensions) const {
