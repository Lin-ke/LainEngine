...
Ref<Resource> lain::ResourceFormatLoaderJSON::load(const String& p_path, const String& p_original_path, Error* r_error, bool p_use_sub_threads, float* r_progress,
...
if (r_error) {
	...
	Error err = json->parse(FileAccess::get_file_as_string(p_path), Engine::GetSingleton()->is_editor_hint());
	if (err != OK) {
...
void lain::ResourceFormatLoaderJSON::get_recognized_extensions(List<String>* p_extensions) const {
	p_extensions->push_back("json");
}

void lain::ResourceFormatLoaderJSON::get_recognized_resources(List<String>* p_extensions) const {
	p_extensions->push_back("JSON");
}
...
Error lain::ResourceFormatSaverJSON::save(const Ref<Resource>& p_resource, const String& p_path, uint32_t p_flags) {
