...
PagedAllocator<Variant::Pools::BucketSmall, true>
...
Variant::Variant(const Vector<int64_t> &p_int32_array)
{
	type = PACKED_INT64_ARRAY;
	_data.packed_array = PackedArrayRef<int64_t>::create(p_int32_array);
}
...
Variant::Variant(const Signal &p_quaternion) : type(SIGNAL)
{
	memnew_placement(_data._mem, Signal(p_quaternion));
}
Variant::Variant(const Basis &p_basis) : type(BASIS)
{
	_data._basis = (Basis *)Pools::_bucket_medium.alloc();
	memnew_placement(_data._basis, Basis(p_basis));
}
...
{
	return Serializer::write<Variant>(*this).dump();
}
...
// @todo 目前variant的反射有两套（serializer和stringify），合并
