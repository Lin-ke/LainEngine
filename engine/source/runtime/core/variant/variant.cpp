...
Variant::Variant(const lain::AABB &p_aabb) :
		type(AABB) {
	_data._aabb = (lain::AABB *)Pools::_bucket_small.alloc();
...
Variant::Variant(const GObjectPath &p_GOBJECT_PATH) :
		type(GOBJECT_PATH) {
	memnew_placement(_data._mem, GObjectPath(p_GOBJECT_PATH));
}

Variant::Variant(const lain::RID &p_rid) :
		type(RID) {
...
/* helpers */
Variant::Variant(const Vector<lain::RID> &p_array) :
		type(ARRAY) {
