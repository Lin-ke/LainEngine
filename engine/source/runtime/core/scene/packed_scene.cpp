...
#define GOBJ_FROM_ID(p_name, p_id)                     \
  ...
  for (int i = 0; i < nc; ++i) {
    ...
    if (i > 0) {
      ...
      ERR_FAIL_COND_V_MSG(n.type == TYPE_INSTANTIATED && base_scene_idx < 0, nullptr,
    ...
    if (i == 0 && base_scene_idx >= 0) {
      ...
      gobj = sdata->instantiate(p_edit_state == GEN_EDIT_STATE_DISABLED ? PackedScene::GEN_EDIT_STATE_DISABLED
                                                                        : PackedScene::GEN_EDIT_STATE_INSTANCE);  //only main gets main edit state
...
#ifdef DEBUG_ENABLED
        if (!gobj) {
          WARN_PRINT(String("GObject '" + String(ret_gobjects[0]->get_path_to(parent)) + "/" + String(snames[n.name]) +
                            "' was modified from inside an instance, but it has vanished.")
                         .ascii()
                         .get_data());
        }
#endif
...
Error SceneState::_parse_gobject(GObject* p_owner, GObject* p_node, int p_parent_idx, HashMap<StringName, int>& name_map,
                                 HashMap<Variant, int, VariantHasher, VariantComparator>& variant_map, HashMap<GObject*, int>& gobject_map,
                                 HashMap<GObject*, int>& nodepath_map) {
  ...
  if (p_owner->get_scene_inherited_state().is_null() &&
      (p_node == p_owner || (p_node->get_owner() == p_owner && (p_node->get_parent() == p_owner || p_node->get_parent()->get_owner() == p_owner)))) {
