...
#define REFLECTABLE_TYPE(class_name) friend class Serializer;
...
#define MANUAL_REFLECTION_BODY(class_name) friend class Serializer;
...
#define Lain_REFLECTION_DEEP_COPY(type, dst_ptr, src_ptr) *static_cast<type*>(dst_ptr) = *static_cast<type*>(src_ptr.getPtr());
#define SERIALIZER(CLASS)                                                                                                                                                    \
  ...
  class Type##CLASS##SerilizerWrapper {                                                                                                                                      \
   public:                                                                                                                                                                   \
    ...
    static size_t get_size_of() {                                                                                                                                            \
      return sizeof(CLASS);                                                                                                                                                  \
    }                                                                                                                                                                        \
    static void* memnew_class(void* target = nullptr) {                                                                                                                      \
      if (target != nullptr) {                                                                                                                                               \
        memnew_placement(target, CLASS);                                                                                                                                     \
        return target;                                                                                                                                                       \
      }                                                                                                                                                                      \
      return memnew(CLASS);                                                                                                                                                  \
    }                                                                                                                                                                        \
    static void* memnew_class_arr(int size_of_arr) {                                                                                                                         \
      return memnew_arr(CLASS, size_of_arr);                                                                                                                                 \
    }                                                                                                                                                                        \
  };                                                                                                                                                                         \
  }                                                                                                                                                                          \
                                                                                                                                                                             \
  ...
  void ClassFunction_Manual_##CLASS##() {                                                                                                                                    \
    ClassFunctionTuple* class_function_tuple_##CLASS## = memnew(ClassFunctionTuple(                                                                                          \
        nullptr, &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::constructorWithJson, &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::writeByName, \
        &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::json_read_warpper, &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::memnew_class,           \
        &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::memnew_class_arr, &TypeFieldReflectionOperator::Type##CLASS##SerilizerWrapper::get_size_of));           \
...
#define REFLECT_FIELD_DEF(CLASS, FName, TYPE, SetF, GetF, IsArray)                                                                                                   \
  ...
  namespace Type##CLASS##Operator {                                                                                                                                  \
    const char* get_fieldname_##FName() {                                                                                                                            \
      return #FName;                                                                                                                                                 \
    }                                                                                                                                                                \
    const char* get_field_typename_##FName() {                                                                                                                       \
      return #TYPE;                                                                                                                                                  \
    }                                                                                                                                                                \
    bool isArray_##FName() {                                                                                                                                         \
      return IsArray;                                                                                                                                                \
  ...
  static void TypeWrappersRegister##CLASS####FName##() {                                                                                                             \
    FieldFunctionTuple* field_function_tuple_##FName = memnew(FieldFunctionTuple(                                                                                    \
        &SetF, &GetF, &TypeFieldReflectionOperator::Type##CLASS##Operator::getClassName, &TypeFieldReflectionOperator::Type##CLASS##Operator::get_fieldname_##FName, \
        &TypeFieldReflectionOperator::Type##CLASS##Operator::get_field_typename_##FName, &TypeFieldReflectionOperator::Type##CLASS##Operator::isArray_##FName));     \
  }                                                                                                                                                                  \
  }                                                                                                                                                                  \
  }
...
#define REFLECT_FIELD_DEF_SAFETY(CLASS, FName, TYPE, SetF, GetF, IsArray)                                                                                            \
  static const char* get_fieldname_##FName() {                                                                                                                       \
    return #FName;                                                                                                                                                   \
  }                                                                                                                                                                  \
  static const char* get_field_typename_##FName() {                                                                                                                  \
    return #TYPE;                                                                                                                                                    \
  }                                                                                                                                                                  \
  static bool isArray_##FName() {                                                                                                                                    \
    return IsArray;                                                                                                                                                  \
  }                                                                                                                                                                  \
  static void TypeWrappersRegister##CLASS####FName##() {                                                                                                             \
    FieldFunctionTuple* field_function_tuple_##FName = memnew(FieldFunctionTuple(                                                                                    \
...
#define GENERATE_MANUAL_REFLECT_FIELD_CODE2(CLASS, FName, ...) \
  Reflection::TypeWrappersRegister##CLASS####FName##();        \
  GENERATE_MANUAL_REFLECT_FIELD_CODE1(CLASS, __VA_ARGS__)
#define GENERATE_MANUAL_REFLECT_FIELD_CODE3(CLASS, FName, ...) \
  Reflection::TypeWrappersRegister##CLASS####FName##();        \
  GENERATE_MANUAL_REFLECT_FIELD_CODE2(CLASS, __VA_ARGS__)
#define GENERATE_MANUAL_REFLECT_FIELD_CODE4(CLASS, FName, ...) \
  Reflection::TypeWrappersRegister##CLASS####FName##();        \
  GENERATE_MANUAL_REFLECT_FIELD_CODE3(CLASS, __VA_ARGS__)
#define GENERATE_MANUAL_REFLECT_FIELD_CODE5(CLASS, FName, ...) \
  Reflection::TypeWrappersRegister##CLASS####FName##();        \
  GENERATE_MANUAL_REFLECT_FIELD_CODE4(CLASS, __VA_ARGS__)
...
#define TypeMetaDef(class_name, ptr) lain::Reflection::ReflectionInstance(lain::Reflection::TypeMeta::newMetaFromName(#class_name), (class_name*)ptr)

#define TypeMetaDefPtr(class_name, ptr) memnew(lain::Reflection::ReflectionInstance(lain::Reflection::TypeMeta::newMetaFromName(#class_name), (class_name*)ptr))
