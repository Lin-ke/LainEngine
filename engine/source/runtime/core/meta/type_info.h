...
struct TypeInherits {
  ...
  static bool const value = sizeof(test(get_d())) == sizeof(char) && !types_are_same_v<B volatile const, void volatile const>;
};
...
#define MAKE_TYPE_INFO_WITH_META(m_type, m_var_type, m_metadata) \
  ...
  struct GetTypeInfo<m_type> {                                   \
    static const Variant::Type VARIANT_TYPE = m_var_type;        \
    static const Reflection::Metadata METADATA = m_metadata;     \
    static inline PropertyInfo get_class_info() {                \
      return PropertyInfo(VARIANT_TYPE, String());               \
    }                                                            \
  };                                                             \
...
MAKE_TYPE_INFO_WITH_META(uint8_t, Variant::INT, Reflection::METADATA_INT_IS_UINT8)
MAKE_TYPE_INFO_WITH_META(int8_t, Variant::INT, Reflection::METADATA_INT_IS_INT8)
MAKE_TYPE_INFO_WITH_META(uint16_t, Variant::INT, Reflection::METADATA_INT_IS_UINT16)
MAKE_TYPE_INFO_WITH_META(int16_t, Variant::INT, Reflection::METADATA_INT_IS_INT16)
MAKE_TYPE_INFO_WITH_META(uint32_t, Variant::INT, Reflection::METADATA_INT_IS_UINT32)
MAKE_TYPE_INFO_WITH_META(int32_t, Variant::INT, Reflection::METADATA_INT_IS_INT32)
MAKE_TYPE_INFO_WITH_META(uint64_t, Variant::INT, Reflection::METADATA_INT_IS_UINT64)
MAKE_TYPE_INFO_WITH_META(int64_t, Variant::INT, Reflection::METADATA_INT_IS_INT64)
MAKE_TYPE_INFO(char16_t, Variant::INT)
MAKE_TYPE_INFO(char32_t, Variant::INT)
MAKE_TYPE_INFO_WITH_META(float, Variant::FLOAT, Reflection::METADATA_REAL_IS_FLOAT)
MAKE_TYPE_INFO_WITH_META(double, Variant::FLOAT, Reflection::METADATA_REAL_IS_DOUBLE)
...
MAKE_TYPE_INFO(GObjectPath, Variant::GOBJECT_PATH)
...
struct GetTypeInfo<ObjectID> {
	...
	static const Reflection::Metadata METADATA = Reflection::METADATA_INT_IS_UINT64;
	static inline PropertyInfo get_class_info() {
...
namespace Reflection{
  // 类中枚举，最后形式为 class_name.enum_name 
  // 问题：无法进行类内部类的枚举?
