...
struct _NO_DISCARD_ Vector3 {
  ...
  friend Vector3 operator+(const Vector3& lhs, real_t rhs) { return Vector3(lhs.x + rhs, lhs.y + rhs, lhs.z + rhs); }

  friend Vector3 operator+(real_t lhs, const Vector3& rhs) { return Vector3(lhs + rhs.x, lhs + rhs.y, lhs + rhs.z); }
  ...
  real_t distance_to(const Vector3& rhs) const { return (*this - rhs).length(); }
  ...
  real_t dot(const Vector3& p_with) const { return x * p_with.x + y * p_with.y + z * p_with.z; }
  ...
  Vector3 Vector3::cross(const Vector3& p_with) const {
    Vector3 ret((y * p_with.z) - (z * p_with.y), (z * p_with.x) - (x * p_with.z), (x * p_with.y) - (y * p_with.x));
  ...
  L_INLINE bool is_zero_approx() const { return Math::is_zero_approx(x) && Math::is_zero_approx(y) && Math::is_zero_approx(z); }
  ...
  /** Calculates a reflection vector to the plane with the given normal .
        ...
        */
  Vector3 reflect(const Vector3& normal) const { return Vector3(*this - (2 * this->dot(normal) * normal)); }
  ...
  bool is_equal_approx(const Vector3& p_v) const { return Math::is_equal_approx(x, p_v.x) && Math::is_equal_approx(y, p_v.y) && Math::is_equal_approx(z, p_v.z); }

  bool is_finite() const { return Math::is_finite(x) && Math::is_finite(y) && Math::is_finite(z) && !is_nan(); }
  bool is_nan() const { return Math::isNan(x) || Math::isNan(y) || Math::isNan(z); }
