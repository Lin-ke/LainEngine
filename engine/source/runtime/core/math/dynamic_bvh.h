 ...
 private:
  struct Volume {
    ...
    _FORCE_INLINE_ bool contains(const Volume& a) const {
      return ((min.x <= a.min.x) && (min.y <= a.min.y) && (min.z <= a.min.z) && (max.x >= a.max.x) && (max.y >= a.max.y) && (max.z >= a.max.z));
    }
    ...
    _FORCE_INLINE_ int select_by_proximity(const Volume& a, const Volume& b) const { return (get_proximity_to(a) < get_proximity_to(b) ? 0 : 1); }
    ...
    _FORCE_INLINE_ bool intersects_convex(const Plane* p_planes, int p_plane_count, const Vector3* p_points, int p_point_count) const {
      ...
      for (int i = 0; i < p_plane_count; i++) {
        ...
        Vector3 point((p.normal.x > 0) ? -half_extents.x : half_extents.x, (p.normal.y > 0) ? -half_extents.y : half_extents.y,
  ...
  struct Node {
    ...
    bool is_left_of_axis(const Vector3& org, const Vector3& axis) const { return axis.dot(volume.get_center() - org) <= 0; }
  ...
  _FORCE_INLINE_ bool _ray_aabb(const Vector3& rayFrom, const Vector3& rayInvDirection, const unsigned int raySign[3], const Vector3 bounds[2], real_t& tmin,
