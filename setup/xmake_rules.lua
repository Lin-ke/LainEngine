...
function public_dependency(dep, version, setting)
    ...
    add_values("l.module.public_dependencies", dep)
...
rule("l.component")
    on_config(function (component, opt)
        ...
        local owner_name = component:extraconf("rules", "l.component", "owner")
        local owner = project.target(owner_name)
        local owner_api = owner:extraconf("rules", "l.dyn_module", "api") or owner:extraconf("rules", "l.static_module", "api")
        -- add dep values to owner
        for _, pub_dep in pairs(component:values("l.module.public_dependencies")) do
            owner:add("values", "l.module.public_dependencies", pub_dep)
        ...
        for _, owner_dep in pairs(owner:orderdeps()) do
            local _owner_name = owner_dep:extraconf("rules", "l.component", "owner") or ""
        ...
        for _, owner_inc in pairs(owner:get("packages")) do
            component:add("packages", owner_inc, {public = true})
        end
        ...
        if owner_api then
            if has_config("shipping_one_archive") then
                ...
                component:add("defines", owner_api.."_API=l_IMPORT", owner_api.."_LOCAL=error")
            end
        end
    end)
rule_end()
...
rule("l.dyn_module")
    ...
    on_load(function (target, opt)
        local api = target:extraconf("rules", "l.dyn_module", "api")
        local version = target:extraconf("rules", "l.dyn_module", "version")
        target:add("values", "l.module.version", version)
        target:add("defines", api.."_IMPL")
        target:add("defines", api.."_EXTERN_C=l_EXTERN_C", {public=true})
        if has_config("shipping_one_archive") then
            target:add("defines","SHIPPING_ONE_ARCHIVE")
            target:add("defines", api.."_API=", {public=true})
        else
            target:add("defines", api.."_SHARED", {public=true})
            target:add("defines", api.."_API=l_IMPORT", {interface=true})
            target:add("defines", api.."_API=l_EXPORT", {public=false})
        end
        ...
        if (target:rule("c++.unity_build")) then
            ...
            unity_build:add("deps", "l.dyn_module", {order = true})
            target:rule_add(unity_build)
        end
        if (target:rule("c.unity_build")) then
            local cunity_build = target:rule("c.unity_build"):clone()
            cunity_build:add("deps", "l.dyn_module", {order = true})
    ...
    on_config(function(target)
        ...
        -- calculate deps
        local api = target:extraconf("rules", "l.dyn_module", "api")
        local dep_modules = module_codegen.resolve_l_module_dependencies(target)
        if has_config("shipping_one_archive") then
        ...
        module_codegen.l_module_gen_json(target, target:data("module.meta.json"), dep_modules)
        module_codegen.l_module_gen_cpp(target, target:data("module.meta.cpp"), dep_modules)
    end)
rule_end()

rule("l.static_library")
    on_load(function (target, opt)
...
rule("l.static_module")
    add_deps("l.module")
    add_deps("l.static_library")
    on_load(function (target, opt)
        local api = target:extraconf("rules", "l.static_module", "api")
...
function static_library(name, api, version, opt)
    target(name)
        ...
        add_rules("l.static_library", { api = api, version = version }) 
        set_kind("static")
        opt = opt or {}
...
function static_module(name, api, version, opt)
    target(name)
        ...
        add_rules("l.static_module", { api = api, version = version }) 
        set_kind("static")
        opt = opt or {}
        if opt.exception and not opt.noexception then
            set_exceptions("cxx")
...
function shared_module(name, api, version, opt)
    target(name)
        set_group("01.modules/"..name)
        add_rules("l.dyn_module", { api = api, version = version }) 
        if has_config("shipping_one_archive") then
            set_kind("static")
        else
            set_kind("shared")
...
function static_component(name, owner, opt)
    ...
    target(name)
        ...
        add_rules("l.component", { owner = owner })
        add_deps(owner)
        set_kind("static")
end
...
function executable_module(name, api, version, opt)
    target(name)
        set_kind("binary")
        add_rules("l.dyn_module", { api = api, version = engine_version }) 
        opt = opt or {}
        if opt.exception and not opt.noexception then
            set_exceptions("cxx")
        else
